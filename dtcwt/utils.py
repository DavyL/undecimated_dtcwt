...
def asfarray(X):
    """Similar to :py:func:`numpy.asfarray` but preserves existing floating or
    complex dtypes and avoids unnecessary copies. Non-floating inputs (ints,
    bools, etc.) are cast to float64. Compatible with NumPy >= 2.0."""
    X = np.asanyarray(X)
    dt = X.dtype
